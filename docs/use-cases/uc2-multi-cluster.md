# Use Case 2: Multi-Cluster

## Overview

**Services exercised:** boot-service, metadata-service

**What it asserts:** Two clusters with disjoint node sets; move a node between them; membership reflects on both services.
...
The CI workflow runs this use case as job `test-uc2`. You can view the results in the [Actions tab](../../actions/workflows/ci.yml).

## Test implementation

- **File:** `tests/integration/uc2_multi_cluster_test.go`
- **Test function:** `TestUC2_*`
...
2. Download the `uc2-logs-*` artifact from the failed run.
